...
let cubeEngine;
let state = "WWWWRRRRGGGGYYYYOOOOBBBB";
let speed = 300;
let scrambleHistory = [];
let currentSolution = [];
...
function updateUI() {
    ...
    document.getElementById('asciiCube').innerHTML =
        `      ${c(s[0])} ${c(s[1])}
      ${c(s[2])} ${c(s[3])}
...
function resetSolutionStrip() {
    ...
    movesContainer.innerHTML = '<span class="solution-placeholder">Scramble the cube, then click Solve</span>';
    countEl.textContent = '';
}
